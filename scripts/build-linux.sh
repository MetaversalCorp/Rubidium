...
#   builds/linux-<arch>/install/{debug,release}/bin/
...
#
#   --deps         Build SDL3 into deps/builds/linux-<arch>/<config>/libs/.
...
# deps/builds/linux-<arch>/{debug,release}/ but share a single Rubidium build
# tree at builds/linux-<arch>/build/ and distinct install trees at
# builds/linux-<arch>/install/{debug,release}/.
#
# Usage:
#   ./scripts/build-linux.sh                      # Rubidium (Release)
#   ./scripts/build-linux.sh --config Debug       # Rubidium (Debug)
#   ./scripts/build-linux.sh --fresh              # Reconfigure only (no build)
#   ./scripts/build-linux.sh --deps               # Deps only
#   ./scripts/build-linux.sh --all                # Deps, then Rubidium
set -euo pipefail
...
# etc.), not Rubidium's SDL3/fonts tree. Forward so `./scripts/build-linux.sh
# --sync` works from the Rubidium repo too.
for _arg in "$@"; do
   if [[ "$_arg" == "--sync" || "$_arg" == "--verify" ]]; then
      if [[ ! -x "$SNEEZE_DIR/scripts/build-linux.sh" ]]; then
         echo "Sneeze build script not found or not executable:" >&2
         echo "  $SNEEZE_DIR/scripts/build-linux.sh" >&2
         echo "Restore it from git in the Sneeze repo:" >&2
         echo "  cd \"$SNEEZE_DIR\" && git checkout HEAD -- scripts/build-linux.sh scripts/build-deps.sh" >&2
         exit 1
      fi
      echo "==> Forwarding to Sneeze (deps sync/verify): $SNEEZE_DIR/scripts/build-linux.sh $*"
      exec "$SNEEZE_DIR/scripts/build-linux.sh" "$@"
   fi
done
...
# Rubidium links Sneeze's deps (BoringSSL, curl, ...), which Sneeze builds with
# clang + libc++. Rubidium MUST use the same toolchain/stdlib or linking the
# libc++-built static libs against a libstdc++ Rubidium fails with undefined
# std::__1 / libc++ runtime symbols. Mirror Sneeze's Linux toolchain exactly.
...
if [[ $DEPS_MODE -eq 1 ]]; then
   echo "==> Linux $PLATFORM deps build ($CONFIG)"
...
if [[ $FRESH -eq 1 || $RUBIDIUM_MODE -eq 1 ]]; then
   ...
   if [[ $FRESH -eq 1 && $REBUILD -eq 0 ]]; then
      ...
      echo "==> Rubidium Linux build complete ($CONFIG)"
      echo "    Rubidium -> $RUBIDIUM_INSTALL_DIR/bin"
   fi
fi
