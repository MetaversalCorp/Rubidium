// Adapted from RmlUi's Backends/RmlUi_Platform_SDL.cpp
...
bool RmlSDL::InputEventHandler(Rml::Context* context, SDL_Window* window, SDL_Event& ev)
{
	...
	// RMLUI_SDL sizes the context in SDL logical (point) coordinates: its
	// dimensions track SDL_GetWindowSize and the visual HiDPI upscale is done by
	// SDL_SetRenderScale(pixel_density). RmlUi therefore lays out and hit-tests
	// in point space, and SDL delivers mouse coordinates in that same point
	// space -- so they must be fed through 1:1. RmlUi's stock backend multiplies
	// by the pixel density because it sizes the context in *pixels* instead;
	// doing that here double-counts DPI on Retina/HiDPI displays and pushes every
	// hit-test toward the lower-right (the window's right/bottom become
	// unclickable). Keep this at 1.0 to match the point-based context.
	...
	constexpr auto event_window_size_changed = SDL_EVENT_WINDOW_RESIZED;
	...
	case event_mouse_down:
	{
		...
		// RmlUi selects a word on double-click but has no triple-click handling.
		// Emulate the familiar "triple-click selects all" by synthesizing a
		// Ctrl+A to the focused element on the third (or later) consecutive
		// click. The preceding ProcessMouseButtonDown has already placed the
		// caret / focused the field, so the select-all lands on the right
		// widget; if nothing editable is focused, RmlUi ignores Ctrl+A.
		if (ev.button.clicks >= 3)
		{
			const int select_all_mods = GetKeyModifierState() | Rml::Input::KM_CTRL;
			context->ProcessKeyDown(Rml::Input::KI_A, select_all_mods);
			context->ProcessKeyUp(Rml::Input::KI_A, select_all_mods);
		}
	}
	break;
	case event_mouse_up:
...
int RmlSDL::GetKeyModifierState()
{
	...
	// On macOS the Command (GUI/Meta) key is the primary editing modifier:
	// Cmd+A / Cmd+C / Cmd+V / Cmd+X for select-all / copy / paste / cut, and
	// Cmd+F for our Elements search. RmlUi's text widgets (and our own shortcut
	// handlers) key all of those off KM_CTRL, so fold Cmd into KM_CTRL here --
	// otherwise those shortcuts silently do nothing on a native Mac, since SDL
	// reports Cmd as GUI, not Ctrl.
	#if SDL_MAJOR_VERSION >= 3
...
// ---------------------------------------------------------------------------
// Rubidium global RmlUi system interface (clipboard + logging + elapsed time).
// Sneeze installs a minimal stub at engine init; App_SDL / App_Win32 replace
// it here after SDL_Init so RmlUi text fields can paste from the OS clipboard.
// ---------------------------------------------------------------------------

namespace RUBIDIUM
{

class RUBIDIUM_RML_SYSTEM : public Rml::SystemInterface
{
public:
   static RUBIDIUM_RML_SYSTEM& Instance ()
   {
      static RUBIDIUM_RML_SYSTEM sInstance;
      return sInstance;
   }
   ...
   double GetElapsedTime () override
   {
      ...
      return std::chrono::duration<double> (tpNow - m_tpStart).count ();
   }
   ...
   bool LogMessage (Rml::Log::Type nType, const Rml::String& sMessage) override
   {
      ...
      if (m_pLogger)
      {
         ::LOGGER::eLOGLEVEL eLevel = ::LOGGER::kLOGLEVEL_Info;

         if (nType == Rml::Log::LT_ERROR)   eLevel = ::LOGGER::kLOGLEVEL_Error;
         if (nType == Rml::Log::LT_WARNING) eLevel = ::LOGGER::kLOGLEVEL_Warning;
         if (nType == Rml::Log::LT_DEBUG)   eLevel = ::LOGGER::kLOGLEVEL_Trace;

         m_pLogger->Log (eLevel, "RmlUi", sMessage);
      }

      return bResult;
   }

   void SetClipboardText (const Rml::String& sText) override
   {
      SDL_SetClipboardText (sText.c_str ());
   }

   void GetClipboardText (Rml::String& sText) override
   {
...
{
   RUBIDIUM_RML_SYSTEM& System = RUBIDIUM_RML_SYSTEM::Instance ();
   System.SetLogger (pLogger);
