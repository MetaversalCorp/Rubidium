...
<div id="elements-container-items" class="list-body"></div>
)rml";

IW_ELEMENTS_CONTAINERS::IW_ELEMENTS_CONTAINERS () :
   m_pItems   (nullptr),
   m_pItemAll (nullptr),
   m_pTree    (nullptr)
{
...
void IW_ELEMENTS_CONTAINERS::SetTree (IW_ELEMENTS_TREE* pTree)
{
   m_pTree = pTree;
}
...
bool IW_ELEMENTS_CONTAINERS::Initialize (Rml::Element* pContainer)
{
   m_pContainer = pContainer;

   m_pContainer->SetInnerRML (s_sRml);

   m_pItems = m_pContainer->GetElementById ("elements-container-items");
...
void IW_ELEMENTS_CONTAINERS::AddContainer (const std::string& sName)
{
   if (sName.empty ())
      return;
...
void IW_ELEMENTS_CONTAINERS::Clear ()
{
   if (m_pItemAll)
...
const std::string& IW_ELEMENTS_CONTAINERS::SelectedContainer () const
{
   return m_sSelected;
}

void IW_ELEMENTS_CONTAINERS::UpdateSelection (Rml::Element* pItem)
{
   for (int n = 0; n < m_pItems->GetNumChildren (); n++)
   {
...
void IW_ELEMENTS_CONTAINERS::ProcessEvent (Rml::Event& Event)
{
   ...
   // Push the selection into the tree: empty == "(all)" (show everything), a
   // non-empty origin shows that fabric's nodes and dims the others (pink).
   if (m_pTree)
      m_pTree->SetOriginFilter (m_sSelected);
}
