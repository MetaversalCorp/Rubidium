...
<div id="container-items" class="list-body"></div>
)rml";

IW_NETWORK_CONTAINERS::IW_NETWORK_CONTAINERS () :
   m_pItems   (nullptr),
   m_pItemAll (nullptr),
   m_pFiles   (nullptr)
{
...
void IW_NETWORK_CONTAINERS::SetFiles (IW_NETWORK_FILES* pFiles)
{
   m_pFiles = pFiles;
}

bool IW_NETWORK_CONTAINERS::Initialize (Rml::Element* pContainer)
{
   m_pContainer = pContainer;

   m_pContainer->SetInnerRML (s_sRml);

   m_pItems = m_pContainer->GetElementById ("container-items");
...
void IW_NETWORK_CONTAINERS::AddContainer (const std::string& sName)
{
   if (sName.empty ())
      return;
...
void IW_NETWORK_CONTAINERS::Clear ()
{
   if (m_pItemAll)
...
const std::string& IW_NETWORK_CONTAINERS::SelectedContainer () const
{
   return m_sSelected;
}

void IW_NETWORK_CONTAINERS::UpdateSelection (Rml::Element* pItem)
{
   for (int n = 0; n < m_pItems->GetNumChildren (); n++)
   {
...
void IW_NETWORK_CONTAINERS::ProcessEvent (Rml::Event& Event)
{
   ...
   // Empty selection ("(all)") clears the origin filter and shows every file.
   if (m_pFiles)
      m_pFiles->FilterContainer (m_sSelected);
}
